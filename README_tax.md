...
.venv/bin/python fetch_tax.py --start 2015 --end 2024     # live OECD fetch + CSVs + charts
.venv/bin/python fetch_tax.py --no-charts                 # data only
...
`data/`, and (by default) renders charts to `outputs/`.
...
Revenue Statistics & Taxing Wages**. Written to `outputs/`:
...
outputs/                        # committable Substack-style figures (with OECD source notes)
